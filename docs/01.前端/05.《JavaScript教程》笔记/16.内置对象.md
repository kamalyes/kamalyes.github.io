...
JavaScript 的所有其它对象都继承自`Object`对象，即那些对象都是`Object`的实例。
...
除了上面提到的两个方法，`Object`还有不少其它静态方法，将在后文逐一详细介绍。
...
上面代码中，`obj.bar`属性指向一个数组，`obj`对象被冻结以后，这个指向无法改变，即无法指向其它值，但是所指向的数组是可以改变的。
...
除了数组作为参数，`concat`也接受其它类型的值作为参数，添加到目标数组尾部。
...
上面代码中，`sort`的参数函数本身接受两个参数，表示进行比较的两个数组成员。如果该函数的返回值大于`0`，表示第一个成员排在第二个成员后面；其它情况下，都是第一个元素排在第二个元素前面。
...
`reduce`方法和`reduceRight`方法依次处理数组的每个成员，最终累计为一个值。它们的差别是，`reduce`是从左到右处理（从第一个成员到最后一个成员），`reduceRight`则是从右到左（从最后一个成员到第一个成员），其它完全一样。
...
`toString`方法只能将十进制的数，转为其它进制的字符串。如果要将其它进制的数，转回十进制，需要使用`parseInt`方法。
...
与其它对象一样，`Number.prototype`对象上面可以自定义方法，被`Number`的实例继承。
...
如果满足分割规则的两个部分紧邻着（即两个分割符中间没有其它字符），则返回数组之中会有一个空字符串。

```js
'a||c'.split('|') // ['a', '', 'c']
```

如果满足分割规则的部分处于字符串的开头或结尾（即它的前面或后面没有其它字符），则返回数组的第一个或最后一个成员是一个空字符串。
...
```js
Date()
// "Tue Dec 01 2015 09:34:53 GMT+0800 (CST)" 字符串类型
```
...
// "Tue Dec 01 2015 09:34:53 GMT+0800 (CST)"
```
...
`Date`实例有一个独特的地方。其它对象求值的时候，都是默认调用`.valueOf()`方法，但是`Date`实例求值的时候，默认调用的是`toString()`方法。这导致对`Date`实例求值，返回的是一个字符串，代表该实例对应的时间。
...
**第三**，参数为年、月、日等多个整数时，年和月是不能省略的，其它参数都可以省略的。也就是说，这时至少需要两个参数，因为如果只使用“年”这一个参数，`Date`会将其解释为毫秒数。
...
日期字符串应该符合 RFC 2822 和 ISO 8061 这两个标准，即`YYYY-MM-DDTHH:mm:ss.sssZ`格式，其中最后的`Z`表示时区。但是，其它格式也可以被解析，请看下面的例子。
...
Date.parse('2011-10-10T15:58:00')
```
...
如果方括号内的第一个字符是`[^]`，则表示**除了字符类之中的字符，其它字符都可以匹配**。比如，`[^xyz]`表示除了`x`、`y`、`z`之外都可以匹配。
...
上面代码中，字符串`bbc news`包含`a`、`b`、`c`以外的其它字符，所以返回`true`；字符串`bbc`不包含`a`、`b`、`c`以外的其它字符，所以返回`false`。

如果方括号内没有其它字符，即**只有`[^]`，就表示匹配一切字符，其中包括换行符**。相比之下，点号作为元字符（`.`）是不包括换行符的。
...
另外，不要过分使用连字符，设定一个很大的范围，否则很可能选中意料之外的字符。最典型的例子就是`[A-z]`，表面上它是选中从大写的`A`到小写的`z`之间52个字母，但是由于在 ASCII 编码之中，大写字母与小写字母之间还有其它字符，结果就会出现意料之外的结果。
...
上面代码中，字符串`foo`，被转成了`"\"foo\""`。这是因为将来还原的时候，内层双引号可以让 JavaScript 引擎知道，这是一个字符串，而不是其它类型的值。
...
如果参数对象有自定义的`toJSON`方法，那么`JSON.stringify`会**使用这个方法的返回值作为参数，而忽略原对象的其它属性**。
...
var user = {
  ...
  toJSON: function () { // 只使用toJSON的返回值作为参数，忽略user对象的其它参数
    return {
...
上面代码中，`JSON.stringify`发现参数对象有`toJSON`方法，就直接使用这个方法的返回值作为参数，而**忽略原对象的其它参数**。
