...
permalink: /pages/0796ba76b5b55368
categories:
  - 《JavaScript教程》笔记
tags:
  - JavaScript
...
##### 除加号与字符串运行会转成字符串外，其它运行基本都会自动转成数值
...
JavaScript 语言标准只提到，`Error`实例对象必须有`message`属性，表示出错时的提示信息，没有提到其它属性。大多数 JavaScript 引擎，对`Error`实例还提供`name`和`stack`属性，分别表示错误的名称和错误的堆栈，但它们是非标准的，不是每种实现都有。
...
### 2、原生错误类型
 ...
 `Error`实例对象是最一般的错误类型，在它的基础上，JavaScript 还定义了其它6种错误对象。也就是说，存在`Error`的6个派生对象。
...
```
 ...
 对于 JavaScript 引擎来说，**遇到`throw`语句，程序就中止了**。引擎会接收到`throw`抛出的信息，可能是一个错误实例，也可能是其它类型的值。
