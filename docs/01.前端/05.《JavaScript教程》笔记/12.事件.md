...
但是，**`stopPropagation`方法只会阻止事件的传播，不会阻止该事件触发`  <p>  `节点的其它`click`事件的监听函数**。也就是说，不是彻底取消`click`事件。
...
#### 3.3 Event.stopImmediatePropagation() 阻止同一个事件的其它监听函数被调用

`Event.stopImmediatePropagation`方法阻止同一个事件的其它监听函数被调用，不管监听函数定义在当前节点还是其它节点。也就是说，该方法阻止事件的传播，比`Event.stopPropagation()`更彻底。

如果同一个节点对于同一个事件指定了多个监听函数，这些函数会根据添加的顺序依次调用。只要其中有一个监听函数调用了`Event.stopImmediatePropagation`方法，其它的监听函数就不会再执行了。
...
- `screenY`：数值，鼠标相对于**屏幕**的垂直位置（单位像素），其它与`screenX`相同。
- `clientX`：数值，鼠标相对于**程序窗口**的水平位置（单位像素），默认值为0，设置该属性不会移动鼠标。
- `clientY`：数值，鼠标相对于**程序窗口**的垂直位置（单位像素），其它与`clientX`相同。
...
下面是一些常用键的字符串形式，其它键请查[文档](https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/code#Code_values)。
...
拖拉的对象有好几种，包括**元素节点、图片、链接、选中的文字等等**。在网页中，除了元素节点默认不可以拖拉，其它（图片、链接、选中的文字）都是可以直接拖拉的。为了让元素节点可拖拉，可以将该节点的`draggable`属性设为`true`。
...
**`dragstart`事件的监听函数，可以用来设置这个属性**。其它事件的监听函数里面设置这个属性是无效的。
...
下面是添加其它类型的数据。由于`text/plain`是最普遍支持的格式，为了保证兼容性，建议最后总是保存一份纯文本格式的数据。
...
基本上，只有一种场合可以监听`unload`事件，其它情况都不应该监听：**用户修改了表单，还没有保存就要离开**。
