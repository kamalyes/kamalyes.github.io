...
在构建应用的静态版本时，我们需要创建一些会重用其它组件的组件，然后通过 *props* 传入所需的数据。*props* 是父组件向子组件传递数据的方式。即使你已经熟悉了 *state* 的概念，也**完全不应该使用 state** 构建静态版本。state 代表了随时间会产生变化的数据。
...
**只保留应用所需的可变 state 的最小集合，其它数据均由它们计算产生。**
...
3. 你能否根据其它 state 或 props 计算出该数据的值？如果是，那它也不是 state。

> **非state的数据**： 
>
> 1. 由父组件传来的
> 2. 不会随时间推移而改变的
> 3. 能根据其它stete或props计算而来的
