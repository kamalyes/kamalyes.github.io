...
对于空值或其它类型的值，`Math.clz32`方法会将它们先转为数值，然后再计算。
...
JavaScript 原生提供`BigInt`对象，可以用作构造函数生成 BigInt 类型的数值。转换规则基本与`Number()`一致，将其它类型的值转为 BigInt。
...
BigInt 对应的布尔值，与 Number 类型一致，即`0n`会转为`false`，其它值转为`true`。
...
比较运算符（比如`>`）和相等运算符（`==`）允许 BigInt 与其它类型的值混合计算，因为这样做不会损失精度。
