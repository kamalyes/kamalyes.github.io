...
```
子组件js：
// 触发自定义事件 向父组件传值， 参数x（可选，传递给父组件的参数，可以是对象或其它）
...
​	如一条博客中，最多有9张图片，这9张图片可和其它数据放在一个记录中。
...
exports.main = async (event, context) => {
  ...
  const result = await cloud.openapi.wxacode.getUnlimited({
    scene: wxContext.OPENID, // 链接参数 不一定传openid，可传其它任意数据，然后通过此数据，在别人扫码进入时就可用于判断
...
// 上面配置规则表示除了player页面被收录外，其它页面不被收录
```
