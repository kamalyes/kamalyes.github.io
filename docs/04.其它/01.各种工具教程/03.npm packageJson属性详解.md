...
项目主页url
**注意:** 这个项目主页url和url属性不同，如果你填写了url属性，npm注册工具会认为你把项目发布到其它地方了，获取模块的时候不会从npm官方仓库获取，而是会重定向到url属性配置的地址。
...
这些模块会在npm link或者npm install的时候被安装，也可以像其它npm配置一样被管理，详见npm的config文档。
