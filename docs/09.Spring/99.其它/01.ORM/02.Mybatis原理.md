...
>
> [完整示例源码地址](https://github.com/kamalyes/spring-tutorial/blob/master/codes/data/spring-data-mybatis/src/main/java/io/github/kamalyes/spring/orm/MybatisDemo.java)
...
<mapper namespace="io.github.kamalyes.spring.orm.mapper.UserMapper">
  <resultMap id="BaseResultMap" type="io.github.kamalyes.spring.orm.entity.User">
  ...
  <insert id="insert" parameterType="io.github.kamalyes.spring.orm.entity.User">
    insert into user (id, name, age,
      address, email)
    values (#{id,jdbcType=BIGINT}, #{name,jdbcType=VARCHAR}, #{age,jdbcType=INTEGER},
      #{address,jdbcType=VARCHAR}, #{email,jdbcType=VARCHAR})
  </insert>
  <update id="updateByPrimaryKey" parameterType="io.github.kamalyes.spring.orm.entity.User">
    update user
...
public class MybatisDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 3. Mapper 映射并执行
        Long params = 1L;
        List<User> list = sqlSession.selectList("io.github.kamalyes.spring.orm.mapper.UserMapper.selectByPrimaryKey", params);
        for (User user : list) {
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210510113446.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210508173040.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210510105641.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210510110638.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210512111723.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210512114852.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210512185709.png)

## SqlSession 内部工作机制

从前文，我们已经了解了，Mybatis 封装了对数据库的访问，把对数据库的会话和事务控制放到了 SqlSession 对象中。那么具体是如何工作的呢？接下来，我们通过源码解读来进行分析。

![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210512173437.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210512150000.png)
...
![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210512160243.png)
...
`io.github.kamalyes.spring.orm.mapper.UserMapper.selectByPrimaryKey` 是配置在 `UserMapper.xml` 的 Statement ID，params 是 SQL 参数。
...
Mybatis 所有的配置信息都维持在 `Configuration` 对象之中。中维护了一个 `Map<String, MappedStatement>` 对象。其中，key 为 Mapper 方法的全限定名（对于本例而言，key 就是 `io.github.kamalyes.spring.orm.mapper.UserMapper.selectByPrimaryKey` ），value 为 `MappedStatement` 对象。所以，传入 Statement Id 就可以从 Map 中找到对应的 `MappedStatement`。

`MappedStatement` 维护了一个 Mapper 方法的元数据信息，其数据组织可以参考下面的 debug 截图：

![img](https://raw.githubusercontent.com/kamalyes/image-bed/master/snap/20210511150650.png)
