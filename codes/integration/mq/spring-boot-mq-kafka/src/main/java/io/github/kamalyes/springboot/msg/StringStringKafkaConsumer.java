package io.github.kamalyes.springboot.msg;
...
public class StringStringKafkaConsumer {
    ...
    @KafkaListener(id = "kafka-string-string-consumer", topics = "${kamalyes.kafka.string-string-topic}")
    public void recvStringStringData(ConsumerRecord<String, String> consumerRecord) {
        log.info("收到kafka消息：{}", consumerRecord.toString());
    }

    @KafkaListener(id = "kafka-string-json-consumer", topics = "${kamalyes.kafka.string-json-topic}")
