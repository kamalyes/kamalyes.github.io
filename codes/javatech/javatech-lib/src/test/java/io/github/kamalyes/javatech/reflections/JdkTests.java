...
/**
 * test reflection symmetry between jrt scanned metadata (Scanners) and java reflection accessibility (ReflectionUtils
 * functions).
...
public class JdkTests {
    ...
    public void checkResources() {
        ...
        mmap.values().forEach(resources ->
            resources.forEach(resource -> {
                ...
                for (URL url : urls) {
                    try { if (!Files.exists(JrtUrlType.getJrtRealPath(url))) diff.add(resource); } catch (Exception e) {
                        diff.add(resource);
                    }
                }
            }));
        System.out.println(Scanners.Resources.index()
            + ": "
            + mmap.values().stream().mapToInt(Set::size).sum()
            + ", missing: "
            + diff.size());
    ...
    private <F extends AnnotatedElement, E extends AnnotatedElement> Map<String, Set<String>> reflect(
        ...
        System.out.println(
            scanner.index() + ": " + mmap.values().stream().mapToInt(Set::size).sum() + ", missing: " + missing.values()
                                                                                                               .stream()
                                                                                                               .mapToInt(
                                                                                                                   Set::size)
                                                                                                               .sum());
                            ...
                            return Files.walk(realPath)
                                        ...
                                        .map(p -> (Vfs.File) new Vfs.File() {
                                            ...
                                            public String getRelativePath() {
                                                return p.startsWith(realPath) ? p.toString()
                                                                                 .substring(
                                                                                     realPath.toString().length())
                                                    : p.toString();
                                            }
